...
export const AuthContextProvider = ({children})=>{                    //children = app component
    const [currentUser,setCurrentUser] = useState(JSON.parse(localStorage.getItem("user")) || null)     // Every thing in localStorage is in string and we have to change it object so we use json.parse
   ...
   useEffect(()=>{                      // Whenever we change current user then local storage also get updated
